 ...
 * Этот компонент реализует выдвижную боковую панель (меню-бургер),
 * служащую для основной навигации в приложении. Панель содержит
 * ссылки на разделы (Заметки, Архив, Корзина), настройки, а также
 * динамически формируемый список тегов с количеством заметок.
 * Компонент управляет навигационным стеком (pageStack), обновляет
 * данные из DatabaseManagerService и обеспечивает плавную
