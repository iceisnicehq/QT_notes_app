 ...
 * Этот файл представляет собой универсальное диалоговое окно подтверждения.
 * Оно отображает заголовок, сообщение и две кнопки ("Подтвердить" и "Отмена").
 * Тексты кнопок и цвет выделения можно настраивать через свойства.
 * Окно появляется с плавной анимацией и затемненным фоном, блокируя
 * взаимодействие с остальным интерфейсом, и отправляет сигналы
 * confirmed() или cancelled() в зависимости от выбора пользователя.
 */
